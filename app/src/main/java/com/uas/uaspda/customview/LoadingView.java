...
public class LoadingView{
    ...
    public void show(){
       ...
       // viewLoading.setBackgroundResource(viewBgResid);
        viewLoading.setBackgroundDrawable(context.getResources().getDrawable(viewBgResid));


        //获得处了标题栏外的部分
        View parentView = ((Activity)context).getWindow().findViewById(Window.ID_ANDROID_CONTENT);
        //初始化弹窗
        popupWindow = new PopupWindow(layoutFile, ActionBar.LayoutParams.WRAP_CONTENT,
                ActionBar.LayoutParams.WRAP_CONTENT);
        //将标题栏外部分的View放入弹窗中
        popupWindow.showAtLocation(layoutFile, Gravity.CENTER, 0, 0);
